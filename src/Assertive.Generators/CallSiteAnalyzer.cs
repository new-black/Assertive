...
namespace Assertive.Generators
{
  ...
  internal static partial class CallSiteAnalyzer
  {
    ...
    /// </summary>
    internal static string? BuildCustomProbe(GeneratorSyntaxContext ctx, ExpressionSyntax core, bool negated,
      OperandCompiler compiler, CancellationToken ct)
    {
      ...
      switch (core)
      {
          ...
          when ctx.SemanticModel.GetSymbolInfo(invocation, ct).Symbol is IMethodSymbol method:
        {
          ...
          parts.Add($"MemberName = {SplitTreeBuilder.Quote(method.Name)}");
          ...
          if (argExpressions.Count > 0)
          {
            var sources = argExpressions.Select(a => SplitTreeBuilder.Quote(a.ToString()));
               ...
               && ctx.SemanticModel.GetSymbolInfo(member, ct).Symbol is IPropertySymbol { IsStatic: false } property:
        {
          ...
          parts.Add($"MemberName = {SplitTreeBuilder.Quote(property.Name)}");
      ...
      OperandCompiler compiler, CancellationToken ct)
    {
      ...
      parts.Add($"InstanceSource = {SplitTreeBuilder.Quote(instance.ToString())}");
